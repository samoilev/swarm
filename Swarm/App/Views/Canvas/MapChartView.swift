...
struct MapChartView: View {
    ...
    private var failureDetail: String {
        if placesFailed {
            return L10n.tr("Не удалось открыть справочник мест. На карте показаны только места с сохранёнными координатами.")
        }
        if vectorsFailed {
            return L10n.tr("Не удалось загрузить контуры материков.")
        }
